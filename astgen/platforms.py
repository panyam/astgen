...
class Java(astgen.ASTPlatform):
    def getType(self, typeobj):
        if type(typeobj) is astgen.BasicType:
            ...
            return "Map<%s,%s>" % (self.getType(typeobj.key_type), self.getType(typeobj.value_type))
        elif type(typeobj) is astgen.EnumType:
            return typeobj.enum_name
        elif type(typeobj) is astgen.PairOf:
            return "Pair<%s,%s>" % (self.getType(typeobj.type1), self.getType(typeobj.type2))
        elif type(typeobj) is astgen.UnionType:
            return "Object"
        return super(Java, self).getType(typeobj)
